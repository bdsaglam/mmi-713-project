...
__global__ void computeL1Distance(float *documents, float *queries, float *output, int N, int D, int Q) {
    ...
    int qIndex = blockIdx.y * blockDim.y + threadIdx.y;
...
int main() {
    ...
    // Initialize document and query data
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < D; i++)
        {
            h_documents[i*D+j] = rand() / (float)RAND_MAX;
        }
    }
    ...
    for (int i = 0; i < Q; i++)
    {
        for (int j = 0; j < D; i++)
        {
            h_queries[i*D+j] = rand() / (float)RAND_MAX;
        }
    }
